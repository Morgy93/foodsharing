...
var server = (0, child_process_1.spawn)('ts-node', ['src/index.ts'], { stdio: 'inherit' });
...
function connect(t, sessionId, cookieName) {
    ...
    var socket = io.connect(WS_URL, {
        ...
        extraHeaders: {
            cookie: (0, cookie_1.serialize)(cookieName, sessionId)
        }
    });
...
function sendMessage(userIds, channel, method, options, callback) {
    superagent.post(HTTP_URL + "/users/".concat(userIds.join(','), "/").concat(channel, "/").concat(method)).send(options).end(callback);
}
...
function addPHPSessionToRedis(userId, sessionId, callback) {
    var _this = this;
    redisClient.set("PHPREDIS_SESSION:".concat(sessionId), 'foo')
        .then(function () { return __awaiter(_this, void 0, void 0, function () { return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, redisClient.sadd("php:user:".concat(userId, ":sessions"), sessionId)];
