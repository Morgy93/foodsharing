...
class StoreModel extends Db
{
	...
	public function update_betrieb($id, $data)
	{
		...
		SET 	`betrieb_status_id` =  ' . (int)$data['betrieb_status_id'] . ',
			...
			`betrieb_kategorie_id` =  ' . ((int)$data['betrieb_kategorie_id'] > 0 ? (int)$data['betrieb_kategorie_id'] : 'NULL') . ',
	...
	public function add_betrieb($data)
	{
		$id = $this->insert('
			...
			' . (int)$data['kette_id'] . ',
			' . ((int)$data['betrieb_kategorie_id'] > 0 ? (int)$data['betrieb_kategorie_id'] : 'NULL') . ',
